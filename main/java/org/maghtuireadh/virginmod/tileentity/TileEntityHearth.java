...
public class TileEntityHearth extends TileEntity implements ITickable
{
	...
	public void update() 
	{
		if (isLit) 
		{
			...
			else if (fuelLevel >= MathHelper.floor(MaxBurn * 0.10) && fuelLevel < MathHelper.floor(MaxBurn * 0.40)) 
			{
			...
			else if (fuelLevel >= MathHelper.floor(MaxBurn * 0.40) && fuelLevel < MathHelper.floor(MaxBurn * 0.70)) 
			{
		...
		Utils.getLogger().info("fuelpitBurnTime: " + fuelLevel + " ashInCount: " + ashInCount + " coalInCount: " + coalInCount + " isStoked: " + isStoked + " isBanked: " + isBanked);
		stokedCheck();
		markDirty();
	}
